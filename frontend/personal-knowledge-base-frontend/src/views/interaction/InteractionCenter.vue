    ...
    async handleNavigate(item) {
      if (Number(item.isRead) === 0) {
        ...
        if (item.interactionType === 'comment') {
          this.stats.commentUnread = Math.max(0, this.stats.commentUnread - 1)
        } else if (item.interactionType === 'like') {
          this.stats.likeUnread = Math.max(0, this.stats.likeUnread - 1)
        } else if (item.interactionType === 'collect') {
          this.stats.collectUnread = Math.max(0, this.stats.collectUnread - 1)
        }
        item.isRead = 1
      }
      if (item.targetUrl) {
